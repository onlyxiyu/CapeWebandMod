...
public class CapeCommand {
    ...
    private static int setCape(CommandSourceStack source, String type) {
        try {
            ...
            source.sendSuccess(() -> Component.literal(PREFIX + "§a已将披风设置为: §e" + capeType.getDisplayName()), false);
            return 1;
        } catch (IllegalArgumentException e) {
            source.sendFailure(Component.literal(ERROR + "§c未知的披风类型: §e" + type));
            showAvailableCapes(source);
            return 0;
        } catch (Exception e) {
            source.sendFailure(Component.literal(ERROR + "§c设置披风时出错: §e" + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int setCustomCape(CommandContext<CommandSourceStack> context) {
        try {
            ...
            context.getSource().sendSuccess(() -> Component.literal(PREFIX + "§a正在设置自定义披风..."), false);
            return 1;
        } catch (Exception e) {
            context.getSource().sendFailure(Component.literal(ERROR + "§c设置自定义披风时出错: §e" + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int toggleCape(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(
            () -> Component.literal(PREFIX + (enabled ? "§a披风已启用" : "§c披风已禁用")),
            false
        );
        return 1;
    }
    ...
    private static int showHelp(CommandContext<CommandSourceStack> context) {
        ...
        source.sendSuccess(() -> Component.literal("§b=== Cape 命令帮助 ==="), false);
        source.sendSuccess(() -> Component.literal("§6/cape set <类型> §7- 设置预定义披风"), false);
        source.sendSuccess(() -> Component.literal("§6/cape toggle §7- 开启/关闭披风显示"), false);
        source.sendSuccess(() -> Component.literal("§6/cape update §7- 更新披风并同步其他玩家披风"), false);
        source.sendSuccess(() -> Component.literal("§6/cape help §7- 显示此帮助信息"), false);
        showAvailableCapes(source);
        return 1;
    }

    private static void showAvailableCapes(CommandSourceStack source) {
        source.sendSuccess(() -> Component.literal("§b=== 可用的披风类型 ==="), false);
    ...
    private static int updateCapes(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        if (context.getSource().getEntity() instanceof ServerPlayer) {
            ...
            try {
                context.getSource().sendSuccess(() -> Component.literal(PREFIX + "§a正在同步披风..."), false);
                ...
                if (ownCapeUrl != null) {
                    ...
                    CapeManager.NETWORK.send(new CapeUpdatePacket(playerName, ownCapeUrl), (Connection) context.getSource().getServer().getPlayerList().getPlayers());
                }
                    ...
                    Minecraft.getInstance().level.players().forEach(player -> {
                        ...
                        if (!otherPlayerName.equals(playerName)) {
                            ...
                            if (capeUrl != null) {
                                ...
                                CapeManager.NETWORK.send(new CapeUpdatePacket(otherPlayerName, capeUrl), (Connection) context.getSource().getServer().getPlayerList().getPlayers());
                            }
                        }
                    });
                }
    ...
    private static int listCapes(CommandContext<CommandSourceStack> context) {
        try {
            ...
            context.getSource().sendSuccess(() -> Component.literal("§b=== 当前披风信息 ==="), false);
            if (currentCape != null) {
                context.getSource().sendSuccess(() -> Component.literal(PREFIX + "§a当前使用的披风: §e" + currentCape), false);
            } else {
                context.getSource().sendSuccess(() -> Component.literal(PREFIX + "§c当前未使用任何披风"), false);
            }
            ...
            context.getSource().sendFailure(Component.literal(ERROR + "§c获取披风信息时出错: §e" + e.getMessage()));
            return 0;
        }
    }
}